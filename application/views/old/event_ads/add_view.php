...
if($event_add_id != 0 ) {
    $full_url = base_url().'event_ads/add/'.$event_add_id;
}
else {
    $full_url = base_url().'event_ads/add/';    
}
...
?>
    <div class="row well">
			<?php if($event_add_id != 0) {?>
					<legend><label class="default_font_color">Create New Event and Exhibition Ads</label>
					...
					<div class="col-lg-4">
						 <?php echo "<select name='event_id' class='hidden'><option value=".$event_add_id." ></option></select>";?>
					</div>
					<div class="col-lg-6">   
				...
				<?php } 
						...
						if(!empty($event['list'])){?>
									<legend><label class="default_font_color">Create New Event Ads</label>
									<input type="submit" name="" value="Submit" class="btn hoverable_btn pull-right" /></legend>
									<div class="col-lg-4">
										<label>Option Name <i class="required"> * </i> </label>
										<input name="ad_name" class="form-control" type="text" value="<?php echo set_value('ad_name'); ?>">
									...
									<div class="col-lg-4">
										<label>Event Name <i class="required"> * </i> </label>
										<select name="event_id" class="form-control">
										
										<?php 
										foreach($event['list']  as $key=>$value)
										{?>
										<option value="<?php echo $event['list'][$key]->event_id;?>"><?php echo $event['list'][$key]->event_name; ?></option>
										<?php }
											?>
										</select>
									</div>
						...
						<?php } 
								...
								<div class="col-md-12 text-center">
									<h3>No Events and Exhibition Found.Please Add Events and Exhibition First</h3>
									<a href="<?php echo base_url()?>events">Go back to Events</a>
								</div>	
						<?php } 
				}?>
