...
if($mall_add_id != 0 ) {
    $full_url = base_url().'mall_ads/add/'.$mall_add_id;
}
else {
    $full_url = base_url().'mall_ads/add/';    
}
...
?>
    ...
    <div class="row well">
	        <?php if($mall_add_id != 0) {?>
				    <legend><label class="default_font_color">Create New Mall Option</label>
				   ...
				   <div class="col-lg-4">
						<?php echo "<select name='mall_id' class='hidden'><option value=".$mall_add_id." ></option></select>";?>
					</div> 
					<div class="col-lg-6">   
			...
			<?php } 
							...
							<div class="col-lg-4">
							  <label>Mall Name <i class="required"> * </i> </label>
								<select name="mall_id" class="form-control">
								 <?php foreach($mall['list']  as $key=>$value)   {?>
									 <option value="<?php echo $mall['list'][$key]->mall_id;?>"><?php echo $mall['list'][$key]->mall_name; ?></option>
								<?php }?>
								</select>
							</div>
							<div class="col-lg-6">   
					...
					<?php }
							...
							<div class="col-md-12 text-center">
								<h3>No Malls Found.Please Add Malls First</h3>
								<a href="<?php echo base_url()?>malls">Go back to Malls</a>
							</div> 
				    <?php }
	        } ?>
        </div>
