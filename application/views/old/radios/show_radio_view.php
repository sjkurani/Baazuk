...
if (isset($msg)) {
 ...
 if (validation_errors()){
	...
	$radio_img_name = (!empty($result['primary'])) ? asset_url()."uploads/radios/".$result['primary'][0]->r_image : '';
?>
...
<div class="section light directory-single">
       <h3 class="show_text main_title "><?php $str = $result['primary'][0]->radio_station_name." - R".$result['primary'][0]->radio_station_id;
       echo ucwords ($str); ?></h3> 
       <h5> <span><?php echo $result['primary'][0]->radio_station_city; ?></span>  </h5>
		<div class="row col-md-12 col-lg-12 col-sm-12">
			<div class="mainfunctionimg2">
			<img src="<?php echo $radio_img_name;?>" />
			</div>
		</div>
		
		<div class="col-md-12 col-sm-12">
			<section style="margin-top:2em;">
                      <h3 class="show_text row"> About Radio</h3>  
                        <ul class="tags">
                          <li  class="tag"> Registered : <?php echo date("d-M-Y", strtotime($result['primary'][0]->created)); ?></li>                      
                        </ul>
                          <h4 class="show_text row">Description</h4>
                <p class="row"> <?php echo $result['primary'][0]->radio_station_desc; ?></p>
            </section>
        </div>
</div>
...
<h3 class="show_text row"> Radio Station options</h3>
	<div class="row ">
		<?php
		$is_autorised = 0;
		...
		if(!empty($result['secondary'] ))
		{
			foreach ($result['secondary']  as $key => $value) 
			{
				...
				$img_url = asset_url()."uploads/radio_ads/".$value->ref_image;
				...
				<a data-target="<?php echo '#'.$read_target; ?>" data-toggle="modal" class="MainNavText" id="MainNavHelp" href="#myModal"><img class="options_img" src="<?php echo $img_url;?>" alt="<?php echo $result['secondary'][$key]->ad_title;?>">
                 <div class="desc"><br>
					<h4>
						...
						<?php $str = $result['secondary'][$key]->ad_title." - R".$result['primary'][0]->radio_station_id."00".$result['secondary'][$key]->ad_id;
       echo ucwords ($str); ?>
					</h4>
				 </div>
					...
					<div class="col-lg-6">
						...
						if($is_autorised) {
							if(in_array($result['secondary'][$key]->ad_id, $saved_array)) {
								...
								$url_str = base_url()."save_ad/save/".$result['secondary'][$key]->ad_id."/radio/".$result['primary'][0]->radio_station_id;
		      	...
		      	<div class="col-md-6 col-lg-6 col-sm-6">
		      		<form action="<?php echo base_url().'enquiries/save' ?>" method="post">
		      			<section class="hidden">
				            ...
				            <input type="text" name="ad_type" value="radios" />
				            <input type="text" name="ad_type_id" value="<?php  echo $result['primary'][0]->radio_station_id; ?>" />
