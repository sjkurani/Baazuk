...
?>
<legend class="text-right"><label><a href="<?php echo base_url().'events/add'; ?>" class='btn btn-warning'>Add New Event and Exhibition</a></label></legend>
	            	...
	            	if(!empty($events['active'])) {
	            		foreach ($events['active'] as $key => $value) {
	            			...
	            			echo "<legend><label><a href=".base_url()."events/show/".$events['active'][$key]->event_id.">".$events['active'][$key]->event_name." -E".$events['active'][$key]->event_id."</a></label>
                                ...
                                <span class='pull-right'><a  class='btn btn-warning' href='events/change_events_status/".$events['active'][$key]->event_id."/2'>Block</a></span>
                               ...
                               <span class='pull-right'><a  class='btn btn-success' href='".base_url()."recommend/save_rec_media/event/".$events['active'][$key]->event_id."'>Recommended</a></span>

                               <span class='pull-right'><a  class='btn btn-success' href='".base_url()."events/view/".$events['active'][$key]->event_id."'>View options</a></span>
	            	...
	            	if(!empty($events['blocked'])) {
	            		foreach ($events['blocked'] as $key => $value) {
	            			...
	            			<label><a href=".base_url()."events/show/".$events['blocked'][$key]->event_id.">".$events['blocked'][$key]->event_name." -E".$events['blocked'][$key]->event_id."</a></label>
                               ...
                               <span class='pull-right'><a  class='btn btn-danger' href='events/delete_events/".$events['blocked'][$key]->event_id."/3'>Delete</a></span>";

                                if($user_type =='admin') 

                                  {
                                    echo " <span class='pull-right'><a  class='btn btn-success' href='events/change_events_status/".$events['blocked'][$key]->event_id."/1'>Reactivate</a></span>";
