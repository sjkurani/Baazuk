<?php
	...
	$event_img_name = (!empty($result['primary'])) ? asset_url()."uploads/events/".$result['primary'][0]->event_image : '';	
?>
...
<div class="section light directory-single">
       ...
       <h3 class="show_text main_title"><?php $str = $result['primary'][0]->event_name." - E".$result['primary'][0]->event_id;
       echo ucwords ($str); ?></h3> 
       <h5> <span><?php echo $result['primary'][0]->event_cityname; ?></span>  </h5>
		<div class="row col-md-6 col-lg-6 col-sm-6">
			<div class="mainfunctionimg1">
			<img src="<?php echo $event_img_name;?>" />
			</div>
		</div>
		...
		<div class="col-md-12 col-sm-12">
			<section style="margin-top:2em;">
                      <h3 class="show_text row"> About Events and Exhibition</h3>  
                        <ul class="tags">
                           <li class="tag">Event Type : <?php echo $result['primary'][0]->event_type; ?></li>
                            <li  class="tag">Start Date : <?php echo date("d-M-Y", strtotime($result['primary'][0]->start_date)); ?></li>
                          <li class="tag"> End Date:<?php echo date("d-M-Y", strtotime($result['primary'][0]->end_date)); ?></li>
                          <li class="tag" > Registered : <?php echo date("d-M-Y", strtotime($result['primary'][0]->created)); ?></li>                      
                        </ul>
                          <h4 class="show_text row">Description</h4>
                 <p class="row"> <?php echo $result['primary'][0]->event_desc; ?></p>
            </section>    
		</div>
</div>
	
	<h3 class="show_text row"> Events and Exhibition options</h3>
	...
	<div class="row ">
		...
		if(!empty($result['secondary'] ))
		{
			foreach ($result['secondary']  as $key => $value) 
			{
				...
				$img_url = asset_url()."uploads/event_ads/".$value->ref_image;
				...
				$price_string = (($result['secondary'][$key]->price_setting != 0))  ? "<span class=' '>Rs.".$value->price." </span>" : "<span class='text-danger'>Enquire for Pricing</span>";
				$desc = $value->ad_desc;

					//	print_r($result['secondary'][$key]->ad_id);
					//	echo "<br>";
					//	print_r($availability_string);
					//	exit();
	   	...
	   	<div class="row col-sm-4 col-lg-4 col-md-4">
			<div class="thumbnail1">
				<a data-target="<?php echo '#'.$read_target; ?>" data-toggle="modal" class="MainNavText" id="MainNavHelp" href="#myModal"><img class="options_img" src="<?php echo $img_url;?>" alt="<?php echo $result['secondary'][$key]->ad_title;?>">
                 ...
                 <div class="desc"><br>
					<h4><?php $str = $result['secondary'][$key]->ad_title." - E".$result['primary'][0]->event_id."00".$result['secondary'][$key]->ad_id;
       echo ucwords ($str); ?></h4>
				 </div>
				</a>
				...
				<div class="row">
					...
					if($is_autorised) {
						if(in_array($result['secondary'][$key]->ad_id, $saved_array)) {
							...
							$url_str = base_url()."save_ad/save/".$result['secondary'][$key]->ad_id."/event/".$result['primary'][0]->event_id;
		      	...
		      	<div class="col-md-6 col-lg-6 col-sm-6">
		      		<form action="<?php echo base_url().'enquiries/save' ?>" method="post">
		      			<section class="hidden">
				            ...
				            <input type="text" name="ad_type" value="events" />
				            <input type="text" name="ad_type_id" value="<?php  echo $result['primary'][0]->event_id; ?>" />
	...
	<div class="col-md-12 text-center"> 
     ...
     <div class="col-sm-12 col-md-12 col-lg-12 media_desc">
        <section>	
	...
	<div id="login" class="modal fade" role="dialog">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">&times;</button>
					<h4 class="modal-title">Log in to enquiry</h4>
				</div>
				<div class="modal-body row">
	...
	<script>
		...
		var lat_lng=arr[0].event_location;
        var map;
        var temp ;
		temp = lat_lng;
		temp = temp.split(",");
