...
echo "</ul>";
    ...
    foreach ($recomanded['mall'] as $key => $value) {
        $saved_recomanded[] = $value->mall_id;
    }
?>
<legend class="text-right"><label><a href="<?php echo base_url().'malls/add'; ?>" class='btn hoverable_btn'>Add New Mall</a></label></legend>
                            ...
                            echo "<legend>
                             <a href=".base_url()."malls/show/".$malls['new'][$key]->mall_id.">".$malls['new'][$key]->mall_name." -M".$malls['new'][$key]->mall_id."</a></label>
                               ...
                               if($user_type =='admin') 
                                  ...
                                  {
                                    echo " <span class='pull-right'><a  class='btn btn-success' href='".base_url()."malls/change_malls_status/".$malls['new'][$key]->mall_id."/1'>Activate</a></span>";
                    ...
                    if(!empty($malls['active'])) {
                        foreach ($malls['active'] as $key => $value) {
                            ...
                            <label><a href=".base_url()."malls/show/".$malls['active'][$key]->mall_id.">".$malls['active'][$key]->mall_name." -M".$malls['active'][$key]->mall_id."</a></label>
                              ...
                              <span class='pull-right'><a  class='btn btn-warning' href='".base_url()."malls/change_malls_status/".$malls['active'][$key]->mall_id."/2'>Block</a></span>

                              <span class='pull-right'><a  class='btn btn-danger' href='".base_url()."malls/delete_malls/".$malls['active'][$key]->mall_id."/3'>Delete</a></span>";

                                if(in_array($malls['active'][$key]->mall_id, $saved_recomanded)) {
                                    echo "<span class='pull-right'><a class='btn btn-success center-block'  href='#'> Recommended </a></span>";
                                }
                                else {
                                    $url_str = base_url()."recommend/save_rec_media/mall/".$malls['active'][$key]->mall_id;
                                    echo "<span class='pull-right'><a class='btn btn-warning center-block'  href=".$url_str."> Recommend </a></span class='pull-right'>";
                                }

                              echo "<span class='pull-right'><a class='btn btn-success' href='".base_url()."malls/view/".$malls['active'][$key]->mall_id."'>View options</a></span>
                        ...
                        echo "<label class='no_result'>No Active Malls found here.</label>";
                    }
                    ?>
                </section>
                    ...
                    if(!empty($malls['blocked'])) {
                        foreach ($malls['blocked'] as $key => $value) {
                            ...
                            <a href=".base_url()."malls/show/".$malls['blocked'][$key]->mall_id.">".$malls['blocked'][$key]->mall_name." -M".$malls['blocked'][$key]->mall_id."</a></label>
                                ...
                                <span class='pull-right'><a  class='btn btn-danger' href='".base_url()."malls/delete_malls/".$malls['blocked'][$key]->mall_id."/3'>Delete</a></span>";

                                 if($user_type =='admin') 

                                  {
                                    echo " <span class='pull-right'><a  class='btn btn-success' href='".base_url()."malls/change_malls_status/".$malls['blocked'][$key]->mall_id."/1'>Reactivate</a></span>";
                        ...
                        echo "<label class='no_result'>No Blocked Malls found here.</label>";
                    }
                    ?>
                </section>
            </div>
