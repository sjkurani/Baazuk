              ...
              <section  class="tab-pane fade in active" id="newPanel">
	            	...
	            	if(!empty($hoarding_ads['new'])) {
	            		foreach ($hoarding_ads['new'] as $key => $value) {
	            			...
	            			echo "<legend><label>".$hoarding_ads['new'][$key]->h_title." -H".$hoarding_ads['new'][$key]->h_id."</label>
	            		...
	            		echo "<label class='no_result'>No New apartments found here.</label>";
	            	}
	            	?>
	            </section>
	         ...
	         <!--    <section  class="tab-pane fade in active" id="newPanel">
	            	...
	            	if(!empty($hoarding_ads['all'])) {
	            		foreach ($hoarding_ads['all'] as $key => $value) {
	            			echo "<div class='row thumbnail list_view'>";
	            			echo "<legend><label>".$hoarding_ads['all'][$key]->h_title."</label>
	            	...
	            	if(!empty($hoarding_ads['active'])) {
	            		foreach ($hoarding_ads['active'] as $key => $value) {
							...
							echo "<legend><label><a href='".base_url()."hoarding_ads/show/".$hoarding_ads['active'][$key]->h_id."'>".$hoarding_ads['active'][$key]->h_title." -H".$hoarding_ads['active'][$key]->h_id."</label></a>
	            				...
	            				<span class='pull-right'><a  class='btn btn-danger' href='".base_url()."hoarding_ads/delete_hoarding/".$hoarding_ads['active'][$key]->h_id."/3'>Delete</a></span>
                                ...
                                if(in_array($hoarding_ads['active'][$key]->h_id, $saved_recomanded)) {
                                    echo "<span class='pull-right'><a class='btn btn-success center-block'  href='#'> Recommended </a></span>";
                                }
                                else {
                                    $url_str = base_url()."recommend/save_rec_media/hoarding/".$hoarding_ads['active'][$key]->h_id;
	            ...
	            <section  class="tab-pane fade in" id="blockedPanel">
	            	...
	            	if(!empty($hoarding_ads['blocked'])) {
	            		foreach ($hoarding_ads['blocked'] as $key => $value) {
	            			echo "<div class='row thumbnail list_view'>";
	            			echo "<legend><label>".$hoarding_ads['blocked'][$key]->h_title." -H".$hoarding_ads['active'][$key]->h_id."</label>
	            				
	            				<span class='pull-right'><a  class='btn btn-danger' href='".base_url()."hoarding_ads/delete_hoarding/".$hoarding_ads['blocked'][$key]->h_id."/3'>Delete</a></span>";
